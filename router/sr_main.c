...
int main(int argc, char **argv)
{
    ...
    unsigned int tcp_estab_idle_timeout;
    unsigned int tcp_transit_idle_timeout;
    ...
    while ((c = getopt(argc, argv, "hns:v:p:u:t:r:l:T:I:E:R")) != EOF)
    {
        switch (c)
        {
            ...
            case 'n':
                ...
                break;
            case 'I':
                icmp_query_timeout = (uint8_t)atoi(optarg);
                break;
            case 'E':
                tcp_estab_idle_timeout = (uint16_t)atoi(optarg);
                break;
            case 'R':
                tcp_transit_idle_timeout = (uint16_t)atoi(optarg);
                break;
        } /* switch */
    } /* -- while -- */
    ...
    /* Init NAT if -n is specified */
    if(nat)
        sr_nat_init(&sr.nat, icmp_query_timeout, tcp_estab_idle_timeout, tcp_transit_idle_timeout);
